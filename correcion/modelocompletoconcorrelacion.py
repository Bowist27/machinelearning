...
# =========================
def plot_correlation_matrix(csv_path: str, feature_names: List[str], target_name: str):
    """
    Carga el CSV y muestra la matriz de correlación usando pandas y seaborn.
    Esta función se llama UNA SOLA VEZ al inicio del experimento.
    """
    # Leer el CSV con pandas para facilitar el cálculo de correlación
    df = pd.read_csv(csv_path)
    
    # Remapear nombres de columnas si es necesario
    ...
    columns_of_interest = feature_names + [target_name]
    df_numeric = df[columns_of_interest].select_dtypes(include=['float64', 'int64'])
    ...
    plt.title('Correlation Matrix - Student Performance Dataset', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.show()
    plt.close()
    
    # Imprimir correlaciones con el target
    print("\n=== Correlaciones con la variable objetivo (Grades) ===")
